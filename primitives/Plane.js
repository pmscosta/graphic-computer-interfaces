...
class Plane extends CGFobject{
	...
	constructor(scene, nrDivs, dimRatio=1, texRatio=1) 
	{
		super(scene);
		...
		this.spacer = 1 / this.nrDivs;


		let texXRatio = dimRatio/texRatio;
		this.minS = 0 - (texXRatio-1)/2;
		this.maxS = 1 + (texXRatio-1)/2;
	...
	initBuffers()
	{
		...
		*/
		...
		this.texCoords = [];
		this.j_tex = (this.maxT - this.minT) / this.nrDivs;
		this.i_tex = (this.maxS - this.minS) / this.nrDivs;
		...
		for (var j = 0; j <= this.nrDivs; j++) 
		{
			...
			for (var i = 0; i <= this.nrDivs; i++) 
			{
				...
				this.texCoords.push(this.minS + i * this.i_tex, this.minT + j * this.j_tex );
